...
plot_resource <- function(model = m_resource, ymax = 17, title = NA){
  ...
  plot <- t %>% 
    ...
    ggplot() +
    geom_point(aes(x = xaxis, y = fit %>% exp, col = origin),size = 2) +
...
t <- effect('status:fert:light', m_resource) %>% as.data.frame() %>% 
  mutate(xaxis = c(1, 2, 4, 5, 7, 8, 10, 11))
...
ggsave('./fig/s1.pdf', grid.arrange(p_s1a, p_s1b, ncol =2, nrow = 1), 
       height = 6.5/2.54, width = 20/2.54, useDingbats=FALSE)
...
p_light_comp_zz <- t_light_comp_zz %>% 
  ...
  ggplot() +
  geom_point(aes(x = xaxis, y = fit %>% exp, col = status), size = 2) +
  geom_errorbar(aes(ymin = lower %>% exp, ymax = upper %>% exp, x = xaxis, col = status), 
                width = 0,  size = 0.8,
                inherit.aes = FALSE) +
