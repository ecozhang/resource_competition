...
In Changchun, China, Huifei (from Yanjie's group)grew 8 species (4 aliens and 4 natives) in a similar design with additional pairs of alien-alien and native-native.\
...
dat_yj <- dat_yj_raw %>% 
  dplyr::select(Nr., Target_Species, Target_Status, Competition, Competitor_Species, Competitor_Status,
         ...
         Target_Shoot, Competitor_Shoot, Target_Root, Target_IniHeight, Competitor_IniHeight)
...
dat_yj_t <- dat_yj %>% # target individuals
  dplyr::select(-Competitor_Shoot, -Competitor_IniHeight) %>% 
  rename(biomass = Target_Shoot,
         ini_height = Target_IniHeight,
...
dat_yj_c <- dat_yj %>% # competitors
  dplyr::select(-Target_Shoot, -Target_IniHeight) %>% 
  rename(biomass = Competitor_Shoot,
         ini_height = Competitor_IniHeight,
...
```{r}
write.csv(table_in_one, 'table/table_in_one20220116.csv', fileEncoding = "UTF-8")
```
...
### different competition treatment
```{r}
...
t_res_comp <- effect('status * fert * light*comp_treat', m_resource_comp) %>% as.data.frame() %>% 
  mutate(xaxis = rep(c(1, 2, 4, 5, 7, 8, 10, 11), 3))
```
...
# t_res_comp %>% 
#   mutate(origin = status) %>% # rename
#   mutate(comp_treat = ifelse(comp_treat == 'inter', 'Interspecific competition', as.character(comp_treat))) %>% 
#   mutate(comp_treat = ifelse(comp_treat == 'intra', 'Intraspecific competition', as.character(comp_treat))) %>% 
#   mutate(comp_treat = ifelse(comp_treat == 'alone', 'Alone', as.character(comp_treat))) %>% 
#   ggplot() +
#   geom_point(aes(x = xaxis, y = fit %>% exp, col = origin),size = 2.5) +
#   geom_errorbar(aes(ymin = lower %>% exp, ymax = upper %>% exp, x = xaxis, col = origin), 
#                 width = 0,  size = 0.6,
...
#   # scale_y_continuous(limits = c(0, ymax)) + 
#   labs(x = '', y = 'Aboveground biomass [g]\n', col = 'Origin') + theme +
#   facet_wrap(~comp_treat, ncol = 1)
# ggsave('./fig/s4.pdf', 
#        height = 20/2.54, width = 12/2.54, useDingbats=FALSE)
```
...
t_res_comp %>% dplyr::select(status, comp_treat, xaxis) %>% unique
```
...
t_res_comp %>% 
  ...
  geom_errorbar(aes(ymin = lower %>% exp, ymax = upper %>% exp, x = xaxis, col = comp_treat), 
                width = 0,  size = 0.6,
                inherit.aes = FALSE) + 
  scale_x_continuous(breaks = c(2, 6), labels = c('Alien', 'Native'), limits = c(0.5, 7.5)) +
